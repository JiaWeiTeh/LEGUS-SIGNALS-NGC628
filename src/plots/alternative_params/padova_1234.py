...
This one plots a padova track with class 1, 2, 3 and 4 clusters.
...
try:
    qtab, mtab, atab = np.load(path2save+"clusterslug_table_phi073_apn1_padova_1234.npy", allow_pickle = True)
except:
    create_clusterslug_table.create_clusterslug_table(stellarTrack = 'padova')
...
if reRun:
    ...
    np.save(path2save+"PadKroupc1234LHa_QH0_noCons.npy", clusterdata)
   
# If user chooses not to rerun the analysis and use pre-run data,
elif not reRun:
    clusterdata = np.load(path2save+"PadKroupc1234LHa_QH0_noCons.npy", allow_pickle = True)
...
plot_tools.save("PadKroupc1234Fesc")
...
plot_tools.save("PadKroupc1234Fesc_Bayesian")
